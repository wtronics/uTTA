...
+ Why do you use LabVIEW for your GUI?
  ...
  + Update (01.07.2026): As the whole postprocessing toolchain is now based on Python GUIs with TTKbootstrap, I will start working on replacing the LabVIEW GUI in the future releases. First steps were already made when the SCPI driver for the measurement device was build.
+ Why do you use an external battery for the gate driver of the heating current switch? You could also use a power supply!
  + In the beginning I was in fact using another power supply for the gate driver. But I was constantly fighting with common mode issues, that's why I changed this part to a 9V or 12V battery which eliminated all problems completely. 


# A Note on AI

As nowadays everything is full of AI a little sidenote on this topic. This project uses AI to some extend to improve development speed and get creative inputs. Things like the microcontroller software is completely hand written. On the PC side things are a bit different. Here AI is mainly used to generate templates or work on concepts. For example the HTML template for the measurement report was mainly generated by AI, because it is somehow surprisingly capable of generating more or less nice looking frontends. Furthermore, some of the GUI concepts were prototyped by AI. Nevertheless, everything generated was and will always be reviewed by a human to reduce the number or errors (which AI also can do pretty reliably). For example AI loves to use outdated tkinter and TTKbootstrap packages with deprecated syntax.
