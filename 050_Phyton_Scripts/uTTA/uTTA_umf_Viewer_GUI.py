...
class umf_Viewer_App(customtkinter.CTk):
    def __init__(self):
        super().__init__()
        ...
        self.minsize(1480, 850)
        self.title("uTTA umf Viewer Tool")
        self.update()
        ...
        self.frm_meas_data = ctk.CTkFrame(master=self)
        ...
        self.fig = Figure(figsize=(12, 7.5), dpi=screen_dpi)
        #self.fig.tight_layout()
        self.fig.subplots_adjust(left=0.06, bottom=0.075, right=0.97, top=0.96, wspace=0.212, hspace=0.42)
    ...
    def update_plots(self):
        ...
        if FileOpened:
            ...
            G_Plots[0, 0].set_title("Diode Voltages of the full measurement")
            G_Plots[0, 0].set_ylabel('Diode Voltage / [V]')
            G_Plots[0, 0].set_xlabel('Time / [s]')
            G_Plots[0, 0].legend()
            G_Plots[0, 0].grid(which='both')

            G_Plots[0, 1].plot(TimeBaseTotal, ADC[3, :], label="Current")  # Plot some data on the axes.
            G_Plots[0, 1].set_title("Drive current")
            G_Plots[0, 1].set_ylabel('Current / [A]')
            G_Plots[0, 1].set_xlabel('Time / [s]')
            G_Plots[0, 1].legend()
            ...
            G_Plots[1, 0].set_title("Diode Voltages of the cooling section")
            G_Plots[1, 0].set_ylabel('Diode Voltage / [V]')
            G_Plots[1, 0].set_xlabel('Time / [s]')
            G_Plots[1, 0].legend()
            G_Plots[1, 0].grid(which='both')

            G_Plots[1, 1].semilogx(TimeBase_Cooling, ADC_Cooling[3, :], label="Current")  # Plot some data on the axes.
            G_Plots[1, 1].set_title("Drive current in cooling section")
            G_Plots[1, 1].set_ylabel('Current / [A]')
            G_Plots[1, 1].set_xlabel('Time / [s]')
            G_Plots[1, 1].legend()
            G_Plots[1, 1].grid(which='both')

            G_Plots[2, 0].set_title("Thermocouple data of the full measurement")
            ...
            G_Plots[2, 0].set_ylabel('Temperature / [°C]')
            G_Plots[2, 0].set_xlabel('Sample')
            G_Plots[2, 0].legend()
