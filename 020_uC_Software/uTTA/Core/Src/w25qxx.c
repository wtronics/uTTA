...
static inline void cs_on(W25QXX_HandleTypeDef *w25qxx) {
    LL_GPIO_ResetOutputPin(w25qxx->cs_port, w25qxx->cs_pin);
}
...
static inline void cs_off(W25QXX_HandleTypeDef *w25qxx) {
	LL_GPIO_SetOutputPin(w25qxx->cs_port, w25qxx->cs_pin);
}
...
uint32_t w25qxx_read_id(W25QXX_HandleTypeDef *w25qxx) {
    ...
    cs_on(w25qxx);
    buf[0] = W25QXX_GET_ID;
    SPI_Tx_Buffer(w25qxx->spiHandle, buf, 1);
    SPI_Rx_Buffer(w25qxx->spiHandle, buf, 3);
...
uint8_t w25qxx_get_status(W25QXX_HandleTypeDef *w25qxx) {
    ...
    cs_on(w25qxx);
    SPI_Tx_Buffer(w25qxx->spiHandle, &buf, 1);
    SPI_Rx_Buffer(w25qxx->spiHandle, &buf, 1);
...
W25QXX_result_t w25qxx_wait_for_ready(W25QXX_HandleTypeDef *w25qxx) {
    ...
    while ((w25qxx_get_status(w25qxx) & 0x01) == 0x01) { };
...
#else
W25QXX_result_t w25qxx_init(W25QXX_HandleTypeDef *w25qxx, SPI_TypeDef *hspi, GPIO_TypeDef *cs_port, uint16_t cs_pin) {
...
W25QXX_result_t w25qxx_read(W25QXX_HandleTypeDef *w25qxx, uint32_t address, uint8_t *buf, uint32_t len) {
    ...
    if (w25qxx_wait_for_ready(w25qxx) != W25QXX_Ok) {
        return W25QXX_Err;
    }

    cs_on(w25qxx);
    SPI_Tx_Buffer(w25qxx->spiHandle, tx, 4);
    SPI_Rx_Buffer(w25qxx->spiHandle, buf, len);
    cs_off(w25qxx);
...
W25QXX_result_t w25qxx_write(W25QXX_HandleTypeDef *w25qxx, uint32_t address, uint8_t *buf, uint32_t len) {
    ...
    for (uint32_t page = first_page; page <= last_page; ++page) {
        ...
        if (w25qxx_write_enable(w25qxx) == W25QXX_Ok) {
            ...
            cs_on(w25qxx);
            SPI_Tx_Buffer(w25qxx->spiHandle, tx, 4);// size will always be fixed
            SPI_Tx_Buffer(w25qxx->spiHandle, buf + buffer_offset, write_len);// Now write the buffer
            cs_off(w25qxx);
        }
...
W25QXX_result_t w25qxx_chip_erase(W25QXX_HandleTypeDef *w25qxx) {
    if (w25qxx_write_enable(w25qxx) == W25QXX_Ok) {
        ...
        W25QXX_CHIP_ERASE };
        cs_on(w25qxx);
        SPI_Tx_Buffer(w25qxx->spiHandle, tx, 1);
        cs_off(w25qxx);
